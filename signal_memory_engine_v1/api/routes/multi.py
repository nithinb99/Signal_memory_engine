...
def multi_query(req: QueryRequest, bg: BackgroundTasks):
    ...
    for role, qa_chain, store in deps.ROLE_CHAINS:
        try:
            ans = invoke_chain(qa_chain, full_prompt)
        except Exception as he:  # invoke_chain already maps quota → 503; keep going for others
            out[role] = AgentResponse(answer="(LLM temporarily unavailable)", chunks=[], flag="stable", suggestion="No action", trust_score=0.0)
            trace_log(role, req.query, "stable", 0.0, request_id)
            continue

        raw_hits  = store.similarity_search_with_score(req.query, k=req.k)
        events    = [{"content": h[0].page_content, "score": float(h[1])} for h in raw_hits]
        ...
        out[role] = AgentResponse(answer=ans, chunks=chunks, flag=flag, suggestion=suggestion, trust_score=top_score)
